...
#화면 타이틀 설정
pygame.display.set_caption("루해리 게임")
...
balls.append({
    ...
    "to_y":-6, #y축 이동방향
    "init_spd_y": ball_speed_y[0]}) #축 최초 속도
...
while running:
    ...
    for event in pygame.event.get(): # 어떤 이벤트가 발생하였는가 ?
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT: #플레이어를 왼쪽으로 이동
    ...
    #100, 200 -> 180, 160, 140 ...
    #500, 200 => 180, 160, 140 ...
    weapons = [ [w[0], w[1] - weapon_speed] for w in weapons] #무기 위치를 위로 올리는 역할
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                weapon_to_remove = weapon_idx #해당 무기 없이개 위한 값설정
                ball_to_remove = ball_idx #해당 공 없애기 위한 값 설정

                #가장 작은크기의 공이 아니라면 다음 단계의 고응로 나누기
                if ball_img_idx < 3:
                    #현재 공 크기 정보를 가지고 옴
                    ...
                    balls.append({ 
                        ...
                        "to_x": -3, #x축 이동방향, -3 이면 왼쪽으로 , 3이면 오른쪽으로
                        "to_y": -6, #y축 이동방향
                        "init_spd_y": ball_speed_y[ball_img_idx + 1]}) #축 최초 속도
            ...
            continue #안쪽 for 문 조건이 마
        break # 안쪽 for 문에서 브레이크를 만나면 2중 for 문에서 탈출 가능
...
#게임 오버 메시지
msg = game_font.render(game_result, True, (255, 255, 10)) # 노랜색
msg_rect = msg.get_rect(center=(int(screen_width / 2), int(screen_height / 2)))
screen.blit(msg, msg_rect)
pygame.display.update() #게임 화면
